...
- **One leaf per session**: `ObsidianChatView` is a window onto a session, holding no conversation state. It persists `{ sessionId }` via `getState()`/`setState()`, so tabs, split panes, and pop-out windows all survive a restart.
- **Discovery is the switcher, not the tab bar**: sidebar tab bars render icons only, so `getDisplayText()` titles do not draw there — every chat pane looks identical. `SessionSwitcherModal` (`src/ui/session-switcher.ts`) is the primary way to reach a session, and the only way to reach one whose pane is closed. The session title is also shown in the pane's own header. Because the switcher can reach any session, "New chat" rebinds the pane you are looking at instead of opening another — one pane, many conversations. Splitting or popping out a pane still works and each leaf keeps its own session, so concurrent conversations remain visible side by side when you want them.
